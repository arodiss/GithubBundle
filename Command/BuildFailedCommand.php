...
class BuildFailedCommand extends AbstractGithubCommand {

	protected function configure() {
		$this
			->setName('ci:build:failed')
			->setDefinition(array(
			...
			->setDescription('Mark build as failed')
		;
	}
	...
	public function execute(InputInterface $input, OutputInterface $output) {
		$this->getClient()->commentPR(
			$input->getOption("pr"),
			$this->getPhraser()->getPhrase(MessageTypes::BUILD_FAILED_LONG)
		);

		$this->getClient()->updateCommitStatus(
			$input->getOption("sha"),
			CommitStatuses::FAILURE,
			$input->getOption("link"),
			$this->getPhraser()->getPhrase(MessageTypes::BUILD_FAILED_SHORT)
		);
	}
}
